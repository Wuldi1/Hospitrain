...
const hospitals = [
  ...
  {
    id: "hadassah_ein_kerem",
    name: "הדסה - עין כרם (ירושלים)",
    region: "ירושלים",
  },
  {
    id: "hadassah_mount",
...
app.put(
  "/api/drills/public/:drillId/sheets/:sheetId/rows/:rowId",
  (req, res) => {
    const drill = getDrillById(req.params.drillId);
    if (!drill) {
      return res.status(404).json({ error: "Drill not found" });
    }
    const sheet = (drill.sheets || []).find(
      (s) => s.sheetId === req.params.sheetId,
    );
    if (!sheet) {
...
app.post("/api/drills", (req, res) => {
  ...
  if (!name || !hospitalId || !date || !templateId) {
    return res
      .status(400)
      .json({ error: "name, hospitalId, date, and templateId are required" });
  }
...
app.post("/api/drills/:drillId/sheets", (req, res) => {
  ...
  if (sheetIdToDuplicate) {
    const sheetToCopy = drill.sheets.find(
      (s) => s.sheetId === sheetIdToDuplicate,
    );
    if (!sheetToCopy) {
...
app.delete("/api/drills/:drillId/sheets/:sheetId", (req, res) => {
  ...
  const sheetIndex = drill.sheets.findIndex(
    (s) => s.sheetId === req.params.sheetId,
  );
  if (sheetIndex === -1) {
    return res.status(404).json({ error: "Sheet not found" });
